...
module.exports = function (schema, options) {
  ...
  // verify if method names are free for usage
  if (schema.methods[VERIFY_PERMISSIONS]) {
    throw new Error('schema.methods.' + VERIFY_PERMISSIONS + ' in use');
  }
  if (schema.methods[GRANT]) {
    throw new Error('schema.methods.' + GRANT + ' in use');
  }
  if (schema.methods[DENY]) {
    throw new Error('schema.methods.' + DENY + ' in use');
  }
